...
function loadAll$1(input2, iterator, options2) {
...
// src/template.ts
var transformToHandlerCode = (operationCollection) => {
  return operationCollection.map((op) => {
    return [
      `http.${op.method}(\`${op.path}\`, ({ response }) => {`,
      `  return response(${op.status}).${op.response}(${op.example})`,
      "},"
    ].map((str2) => "  ".concat(str2)).join("\n");
  }).join("\n");
};
var mockTemplate = (operationCollection, examples) => `
/**
...
// src/utils.ts
var isRef = (obj) => Object.hasOwn(obj ?? {}, "$ref");
var getRefField = (obj) => {
  return isRef(obj) ? obj.$ref : "";
};
var parseOptions = (options2) => {
  const input2 = options2[0];
  const option = options2.slice(1).flatMap((_, i, a) => {
    return i % 2 ? [] : [a.slice(i, i + 2)];
  }).filter(([key]) => key?.startsWith("--")).map(([key, value]) => [key?.slice(2), value]);
  const entries = { ...Object.fromEntries(option), input: input2 };
...
var getExample = (obj) => {
  ...
  const schema2 = examplePath.split("/").at(-1);
  const example = schema2 ? `examples.${schema2}.value` : "";
...
var pathWithOp = (paths) => (path2) => {
  const methods = Object.keys(paths[path2] ?? {});
  return methods.map((method) => {
    const responses = paths[path2]?.[method]?.responses ?? {
      200: { $ref: "" },
      201: { $ref: "" }
    };
...
var createOpCollection = ({ paths, components }) => {
  const op = pathWithOp(paths);
  const _opCollection = Object.keys(paths).flatMap((path2) => op(path2));
  ...
  return _opCollection.map((op2) => {
    const target = examples.find((e) => e.key === op2.content);
...
var doc = load(readFileSync(input, "utf8"));
main(doc, outdir);
export {
  main
};
